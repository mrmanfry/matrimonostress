...
export function ExpenseLineRow({
  ...
  const getQuantityDisplay = (): number => {
    if (localData.quantity_type === 'fixed') {
      ...
      return calculationMode === 'planned' ? plannedAdults : actualAdults;
    } else if (localData.quantity_type === 'children') {
      return calculationMode === 'planned' ? plannedChildren : actualChildren;
    } else if (localData.quantity_type === 'total_guests') {
      return calculationMode === 'planned' 
  ...
  return (
    <Card className="p-4">
      ...
      {/* Labels Row (solo per riferimento visivo) */}
